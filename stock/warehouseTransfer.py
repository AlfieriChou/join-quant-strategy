...
def set_param():
  ...
  g.sellrank = 8 # 排名多少位之后(不含)卖出
  g.buyrank = 9 # 排名多少位之前(含)可以买入

  # 初始筛选
  g.tradeday = 300 # 上市天数
  g.increase1d = 0.087 # 前一日涨幅
  g.tradevaluemin = 1 # 最小流通市值 单位（亿）
  g.tradevaluemax = 500 # 最大流通市值 单位（亿）
  g.pbmin = 0.01 # 最小市净率
  g.pbmax = 100 # 最大市净率
...
def get_stock_list(context):
  df = get_fundamentals(
    query(valuation.code)
      .filter(valuation.pb_ratio.between(g.pbmin, g.pbmax))
      .filter(valuation.market_cap.between(g.tradevaluemin,g.tradevaluemax))
...
def get_stock_rank_m_m(stock_list):
  rank_stock_list = get_fundamentals(
